...
export const issueBook = async (req, res) => {
    try {
        ...
        const userResponse = await userServiceBreaker.execute(() =>
            axios.get(`${USER_SERVICE_URL}/api/users/${user_id}`, { timeout: 5000 })
        );
        ...
        const bookResponse = await bookServiceBreaker.execute(() =>
            axios.get(`${BOOK_SERVICE_URL}/api/books/${book_id}`, { timeout: 5000 })
        );
        ...
        await bookServiceBreaker.execute(() =>
            axios.patch(`${BOOK_SERVICE_URL}/api/books/${book_id}/availability`, {
                operation: "decrement"
            }, { timeout: 5000 })
        );
...
export const returnBook = async (req, res) => {
    try {
        ...
        await bookServiceBreaker.execute(() =>
            axios.patch(`${BOOK_SERVICE_URL}/api/books/${loan.book_id}/availability`, {
                operation: "increment"
            }, { timeout: 5000 })
        );
...
export const getLoansByUser = async (req, res) => {
    try {
        ...
        for (const loan of loans) {
            const bookResponse = await bookServiceBreaker.execute(() =>
                axios.get(`${BOOK_SERVICE_URL}/api/books/${loan.book_id}`, { timeout: 5000 })
            );
            const book = bookResponse.data;
            loanResponse.push({
                id: loan._id,
    ...
    try {
        ...
        const [userResponse, bookResponse] = await Promise.all([
            userServiceBreaker.execute(() =>
                axios.get(`${USER_SERVICE_URL}/api/users/${loan.user_id}`, { timeout: 5000 })
            ),
...
export const getStatsOverview = async (req, res) => {
    try {
        const [totalBooksResponse, totalUsersResponse, availableBooksResponse] = await Promise.all([
            bookServiceBreaker.execute(() =>
                axios.get(`${BOOK_SERVICE_URL}/api/books/count`, { timeout: 5000 })
            ),
            userServiceBreaker.execute(() =>
                axios.get(`${USER_SERVICE_URL}/api/users/count`, { timeout: 5000 })
            ),
            bookServiceBreaker.execute(() =>
                axios.get(`${BOOK_SERVICE_URL}/api/books/available-count`, { timeout: 5000 })
            )
        ]);
