...
class CircuitBreaker {
  ...
  async execute(requestFn) {
    if (this.state === CircuitBreakerStates.OPEN) {
      ...
      throw new Error(
        `${this.serviceName} service circuit breaker is ${this.state}`
      );
    }
...
export const getActiveUsers = async (req, res) => {
  try {
    ...
    const response = await loanServiceBreaker.execute(() =>
      axios.get(`${loanServiceUrl}/api/loans/active-users`, {
        timeout: 5000,
      })
    );
    ...
    res
      .status(error.message.includes("circuit breaker is OPEN") ? 503 : 500)
      .json({
