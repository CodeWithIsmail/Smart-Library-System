...
export const issueBook = async (req, res) => {
  try {
    ...
    if (!bookResponse.book) {
      return res
        .status(bookResponse.status || 400)
        .json({ message: bookResponse.message || "Book is not available" });
    }
...
export const getLoansByUser = async (req, res) => {
  try {
    const loans = await Loan.find({ user_id: req.params.user_id }).populate(
      "book_id"
    );
...
export const extendLoan = async (req, res) => {
  try {
    ...
    extended_due_date.setDate(
      extended_due_date.getDate() + parseInt(extension_days)
    );
